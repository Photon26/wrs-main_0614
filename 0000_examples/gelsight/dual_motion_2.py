...
for theta in range(3,18):
    rotmat = np.array([[np.cos(np.pi / 48 * theta + np.pi/4), 0, np.sin(np.pi / 48 * theta + np.pi/4)], [0, 1, 0],
                       [-np.sin(np.pi / 48 * theta + np.pi/4), 0, np.cos(np.pi / 48 * theta + np.pi/4)]])
...
ini_pos_lft = ini_pos + np.dot(ini_rot_lft, np.array([0, 0.012, -0.04]))
